...
public class BikeConsole {
	...
	public void printShared(Scanner sc) {
		System.out.println("[ 1. 자전거의 거치율이 0%인 정거장 | 2. 거치율이 150%이상인 정거장 ]");
		int input = Integer.parseInt(sc.nextLine());
	...
	public static void BikeConsoleRun() {
		int selectNum = 1;
		int adminSelectNum = 1;
		BikeConsole mc = new BikeConsole();
		...
		EXIT: while (true) {
			...
			case 4:
				...
				break EXIT;
			case 9999:
				System.out.println("관리자 프로그램을 실행합니다. 비밀번호를 입력해주세요.");
				int pwdCheck = Integer.parseInt(sc.nextLine());
				if(pwdCheck != 1123) {
